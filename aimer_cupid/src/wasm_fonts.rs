...
pub async fn fetch_font_bytes(url: &str) -> Result<Vec<u8>, wasm_bindgen::JsValue> {
    ...
    if !response.ok() {
        return Err(wasm_bindgen::JsValue::from_str(&format!("font fetch failed with status {}", response.status())));
    }
...
pub async fn fetch_and_register_font_bytes(
    ...
    canvas
        ...
        .ok_or_else(|| wasm_bindgen::JsValue::from_str(&format!("failed to register font from {url}")))
}
