...
fn vs_main(
    ...
    // Synthetic italic: shear x by the skew factor, pinned at the quad's bottom
    // edge (c.y == 1 → no shift, c.y == 0 → full shift right), so upright glyphs
    // (skew == 0) are untouched.
