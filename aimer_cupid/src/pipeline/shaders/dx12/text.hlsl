// Hand-authored HLSL for glyph coverage and color conversion.
...
Texture2D<float> t_atlas : register(t1, space0);
...
struct TextVertexIn {
    ...
    float4 clip_radius : TEXCOORD5;
    float skew : TEXCOORD6;
    float coverage_exponent : TEXCOORD7;
};
struct TextVaryings {
    ...
    float4 clip_rect : TEXCOORD3;
    float4 clip_radius : TEXCOORD4;
    float coverage_exponent : TEXCOORD5;
};
TextVaryings vs_main(uint vertex_id : SV_VertexID, TextVertexIn inst) {
    ...
    output.clip_radius = inst.clip_radius;
    output.coverage_exponent = inst.coverage_exponent;
    return output;
}
    ...
    if (rect.z >= 0.0) {
        if (rect.z <= 0.01 || rect.w <= 0.01) {
            ...
            coverage = 1.0 - smoothstep(-0.5, 0.5,
                sdf_rounded_rect(pixel - rect.xy - half_size, half_size, radii));
        } else {
...
float4 fs_main(TextVaryings input) : SV_Target0 {
    float coverage = t_atlas.SampleLevel(s_atlas, input.uv, 0.0);
    if (viewport.surface_is_srgb < 0.5) {
        coverage = pow(max(coverage, 0.0), max(input.coverage_exponent, 0.01));
    }
    float alpha = input.color.a * coverage
        * clip_alpha(input.pixel_pos, input.clip_rect, input.clip_radius);
    float3 rgb = input.color.rgb;
    if (viewport.surface_is_srgb < 1.5) {
        rgb = float3(srgb_to_linear(rgb.r), srgb_to_linear(rgb.g), srgb_to_linear(rgb.b));
    }
    float4 result = float4(rgb * alpha, alpha);
    if (viewport.surface_is_srgb < 0.5 && alpha > 0.00001) {
        result.rgb = float3(linear_to_srgb(rgb.r), linear_to_srgb(rgb.g),
                            linear_to_srgb(rgb.b)) * alpha;
    }
    return result;
}
