...
///
/// Child widgets should provide distinct keys; use [`MorphTransition::child_key`]
...
impl<T: Widget + 'static> Widget for MorphTransitionFrame<T> {
    fn to_element(&self, ctx: &BuildContext) -> Box<dyn Element> {
        ...
        let old_size =
            old_child.as_ref().map(|child| child.computed_size(ctx)).unwrap_or(current_size);
...
impl Drawable for MorphTransitionElement {
    fn draw(&self, ctx: &BuildContext) {
        ...
        if is_animating {
            ...
            *self.morph_state.lock().unwrap() = MorphState::Idle;
        }
    }
}
...
impl EventElement for MorphTransitionElement {
    fn on_event(&self, event: &ElementEvent) -> bool {
        unsafe { self.current_child.get().map(|c| c.on_event(event)).unwrap_or(false) }
    }
...
impl LayoutElement for MorphTransitionElement {
    ...
    fn get_size_from_child(&self) -> Option<Size> {
        unsafe { self.current_child.get().and_then(|c| c.get_size_from_child()) }
    }
