...
//!
//! Reconciliation (`adopt_scroll_state`) only carries a scroll position when the
//! old element still exists to copy from. A *full teardown* — a swapped tab, an
//! `if/else` branch, a list item that unmounts — has no old element, so on the
//! next build the `Scrollable` would re-seed from `ScrollBehavior::scroll_offset`
//! (usually the top). Flutter solves this by writing the offset into a
//! `PageStorageBucket` that lives outside the widget subtree; this store plays
//! the same role, keyed by a `Scrollable`'s explicit `storage_key`.
//!
...
//! map with no lock. Entries are never evicted, so an app that churns through
//! unbounded unique `storage_key`s grows this map without limit. Upgrade path:
//! evict on an explicit dispose hook, or cap it with an LRU.
