...
impl Drawable for BoxDecoration {
    fn draw(&self, ctx: &BuildContext) {
        ...
        // Draw box shadows (outer shadows drawn before background, inset shadows drawn
        // after)
                ...
                .has_visible_outline(box_width, box_height, scale)
        {
            ...
            // Note: fill_rect_with_border_and_outline_per_side currently only supports
            // uniform border/outline color in this API call. If colors are
            // different per side, it would need multiple calls or a more complex shader.
            ...
            ctx.canvas.fill_rect_with_border_and_outline_per_side(
                ...
                [b_widths.1, b_widths.2, b_widths.3, b_widths.0], /* stroke_rect_per_side
                                                                   * uses [top, right,
                                                                   * bottom, left] */
                self.border.effective_color(box_width, box_height, scale), /* uniform color: use
                                                                            * the first visible
                                                                            * side */
...
impl BoxDecoration {
    /// Draws a box shadow using a single GPU draw call with SDF-based Gaussian
    /// blur.
    fn draw_shadow(
        ctx: &BuildContext,
        shadow: &BoxShadow,
