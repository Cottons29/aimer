...
pub struct CallbackRebindVariant;
impl GuestVariant for CallbackRebindVariant {
    const SCHEMA_VERSION: Version = Version::new(2, 0);
    const STATE_TAG: u8 = 2;
    const CALLBACK_STEP: u8 = 10;
    const HEADER: &'static str = "FULL STATE / CALLBACK REBOUND";
    const BUTTON: &'static str = "increment +10 (same callback ID)";
    const TRAP_ON_BUILD: bool = false;
    fn migrate(previous: Version, payload: &[u8]) -> Option<Vec<u8>> { reject_migration(previous, payload) }
}
pub type FullStateGuest = FullStateProgram<CallbackRebindVariant>;
