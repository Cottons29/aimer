...
impl BlogStore {
    pub fn load(root: impl AsRef<Path>) -> Result<Self, String> {
        ...
        for blog in &blogs {
            ...
            if blog.title.trim().is_empty() || blog.upload_time.trim().is_empty() {
...
async fn list_blogs(State(store): State<BlogStore>) -> Json<BlogList> {
    Json(BlogList { blogs: store.0.blogs.clone() })
}
...
mod tests {
    ...
    fn fixture() -> TempDir {
        ...
        fs::write(
            ...
            r#"[
                {"id":"older-post","upload_time":"2026-06-01T10:00:00Z","title":"Older post"},
                {"id":"new-post","upload_time":"2026-07-18T02:22:00Z","title":"New post"}
            ]"#,
        )
        .unwrap();
    ...
    #[tokio::test]
    async fn get_blog_returns_markdown_with_content_type() {
        let root = fixture();
        let response = app(BlogStore::load(root.path()).unwrap())
            .oneshot(
        ...
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(response.headers()[header::CONTENT_TYPE], "text/markdown; charset=utf-8");
        let body = to_bytes(response.into_body(), usize::MAX)
            .await
            .unwrap();
        assert_eq!(body, "# New\n\nHello, Aimer!\n");
    ...
    fn store_rejects_duplicate_ids_and_missing_markdown() {
        ...
        fs::write(
            root.path().join("index.json"),
            r#"[
                {"id":"same","upload_time":"2026-01-01T00:00:00Z","title":"One"},
                {"id":"same","upload_time":"2026-01-02T00:00:00Z","title":"Two"}
            ]"#,
        )
        .unwrap();
        assert!(
        ...
        fs::write(
            root.path().join("index.json"),
            r#"[{"id":"missing","upload_time":"2026-01-01T00:00:00Z","title":"Missing"}]"#,
        )
        .unwrap();
        assert!(
            BlogStore::load(root.path())
