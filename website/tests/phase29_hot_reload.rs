...
fn run_generated_test(
    ...
    let status = Command::new(env!("CARGO"))
        ...
        .env("WEBSITE_PHASE29_STAGE", stage)
        .env("WEBSITE_PHASE29_STATE_PATH", state_path)
        .env("WEBSITE_PHASE29_CALLBACK_IDS_PATH", callback_ids_path)
        .status()
        .unwrap();
    assert!(
        status.success(),
        "{stage} generated website proof failed with status {status}"
    );
}
...
fn build_and_validate_wasm(project: &Path, generated: &GeneratedGuestPackage) {
    ...
    assert!(
        ...
        "website generated wasm build failed\nstdout:\n{}\nstderr:\n{}",
...
fn copy_tree(source: &Path, destination: &Path) {
    ...
    if source == fixture_root() {
        ...
        let workspace = workspace_root();
        let website = workspace.join("website");
        fs::write(
            manifest_path,
            manifest
                .replace("../../../..", &workspace.to_string_lossy())
                .replace("../../..", &website.to_string_lossy()),
        )
        .unwrap();
    }
}
...
use website_phase29_hot_reload_app::{
...
#[test]
fn generated_website_guest_proves_the_public_boundary() {
    if std::env::var("WEBSITE_PHASE29_STAGE").as_deref() == Ok("initial") {
...
fn prove_initial_generation() {
    ...
    assert_contains(&initial, "website: INITIAL @ 1");
    assert_contains(&initial, "sync: 0");
    assert_contains(&initial, "async: 0");

    let bindings = press_bindings(&initial);
    assert_eq!(bindings.len(), 2, "expected one sync and one async website proof button");
...
fn prove_changed_generation_and_recovery() {
    ...
    assert_contains(&initial, "website: UPDATED @ 2");
    assert_contains(&initial, "sync: 0");
    assert_contains(&initial, "async: 0");
    ...
    assert_contains(&recovered, "website: UPDATED @ 2");
    ...
    assert_contains(&restored, "website: UPDATED @ 2");
...
fn state_path() -> PathBuf {
    PathBuf::from(std::env::var("WEBSITE_PHASE29_STATE_PATH").unwrap())
}

fn callback_ids_path() -> PathBuf {
    PathBuf::from(std::env::var("WEBSITE_PHASE29_CALLBACK_IDS_PATH").unwrap())
}
"###;
